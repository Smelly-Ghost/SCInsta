...
%new - (void)handleLongPress:(UILongPressGestureRecognizer *)sender {
    ...
    if ([self respondsToSelector:@selector(imageSpecifier)]) {
        // Using performSelector allows us to grab the object dynamically without strict header checking
        id specifier = [self performSelector:@selector(imageSpecifier)];
        if (specifier && [specifier respondsToSelector:@selector(url)]) {
            photoUrl = [specifier performSelector:@selector(url)];
        }
    }
